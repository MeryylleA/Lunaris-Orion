...
class MultiHeadAttention(nn.Module):
    ...
    def enable_flash_attention(self):
        ...
        if hasattr(F, 'scaled_dot_product_attention'):
            ...
            logging.info("Flash Attention enabled for MultiHeadAttention")
        else:
            logging.warning("Flash Attention not available, using regular attention")
    ...
    def forward(self, 
        ...
        # Scaled dot-product attention with flash attention if enabled
...
class MiniModel(nn.Module):
    ...
    def enable_flash_attention(self):
        """Enable flash attention in all transformer blocks."""
        for block in self.transformer_blocks:
            block.enable_flash_attention()
        logging.info("Flash Attention enabled for all transformer blocks")
        return self
    ...
    def generate(self, tokens, temperature=0.7):
        """Generate an image from input tokens."""
        # Get text embeddings
        text_features = self.forward(tokens)  # [batch_size, seq_len, embedding_dim]
        
        # Apply global average pooling with attention weights
        attention_weights = torch.softmax(text_features @ text_features.transpose(-2, -1) / np.sqrt(text_features.size(-1)), dim=-1)
        text_features = (attention_weights @ text_features).mean(dim=1)  # [batch_size, embedding_dim]
        
        # Project to image dimensions
        batch_size = tokens.size(0)
        image_size = self.config['image_size']
        
        # Add noise based on temperature
        if temperature > 0:
            noise = torch.randn_like(text_features) * temperature
            text_features = text_features + noise
        
        # Generate image features
        image_features = self.to_pixels(text_features)
        
        # Reshape and normalize to [0, 1]
        image = image_features.view(batch_size, 3, image_size, image_size)
        image = torch.sigmoid(image)  # Ensure output is in [0, 1]
